  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-neutral-400 space-y-[2px]">
        ...
        {isAdmin && (
          <DropdownMenuItem className="text-yellow-400 p-2 text-sm cursor-pointer">
            Server Settings <Settings className="w-5 h-5 ml-auto" />
          </DropdownMenuItem>
        )}

        {isAdmin && (
          <DropdownMenuItem className="text-blue-400 p-2 text-sm cursor-pointer">
            Manage Members <Users className="w-5 h-5 ml-auto" />
          </DropdownMenuItem>
        )}

        {isModerator && (
          <DropdownMenuItem className="text-green-400 p-2 text-sm cursor-pointer">
